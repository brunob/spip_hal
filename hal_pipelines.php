...
function hal_optimiser_base_disparus($flux) {
	...
	$res = sql_select(
		'S.id_hal AS id',
		'spip_hals_publications AS S LEFT JOIN spip_hals AS hal ON S.id_hal=hal.id_hal',
		'hal.id_hal IS NULL'
	);
...
function hal_afficher_complement_objet($flux) {
	if ($flux['args']['type'] == 'hal') {
		...
		$flux['data'] .= recuperer_fond('prive/objets/liste/hals_publications', $contexte, ['ajax' => true]);
	} elseif ($flux['args']['type'] == 'auteur') {
		$contexte = array_merge(['id_auteur' => $flux['args']['id']], $_GET);
		$flux['data'] .= recuperer_fond('prive/objets/liste/hals', $contexte, ['ajax' => true]);
	}
	return $flux;
}
